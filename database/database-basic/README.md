...
✅ 사용자나 프로그래머의 특정 요청 발생

✅ 애플리케이션 프로그램에서 쿼리를 날린다.

✅ DBMS이 쿼리를 받아서 분석하고 요청한 것이 무엇인지 파악이 되면

✅ 요청을 처리해야 하는데, 요청을 처리하기 위해서는 요청된 데이터가 어떤 형태로 되어 있는지 부가적인 정보를 알아야 한다.

✅ 확인한 부가적인 정보를 바탕으로 실제 저장된 데이터를 찾아서 애플리케이션에 돌려준다.
...
user application으로 부터 물리적인(physical) database를 분리시키려는 목적으로 이 구조를 사용한다.
...
컨셉츄얼 스키마가 없다는 것은, 익스터널 스키마가 인터널 스키마에 의존하게 된다는 의미이기 때문에,
